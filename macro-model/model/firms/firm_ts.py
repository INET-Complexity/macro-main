...
def create_firms_timeseries(
    ...
    return TimeSeries(
        ...
        unconstrained_target_production=np.full(len(data), np.nan),
        constrained_target_production=np.full(len(data), np.nan),
        ...
        #
        unconstrained_target_intermediate_inputs=np.full((len(data), n_industries), np.nan),
        ...
        total_inventory_change=np.zeros(len(data)),
        #
        ...
        target_long_term_credit=np.zeros(len(data)),
        total_target_long_term_credit=[0.0],
        received_short_term_credit=np.full(len(data), np.nan),
        total_received_short_term_credit=[0.0],
        received_long_term_credit=np.full(len(data), np.nan),
        total_received_long_term_credit=[0.0],
        received_credit=np.full(len(data), np.nan),
        #
        short_term_loan_debt=np.zeros(len(data)),
