...
class DefaultHousingMarketClearer(HousingMarketClearer):
    ...
    def perform_matching(
        ...
        for household_id in households_with_demand_shuffled:
            ...
            if is_rental_market:
                property_open_ind = np.where(housing_data["Up for Rent"] == True)[0]
                property_prices = housing_data.loc[property_open_ind, "Rent"]
            else:
                property_open_ind = np.where(housing_data["Temporarily for Sale"] == True)[0]
            ...
            households_already_operated.append(household_id)
