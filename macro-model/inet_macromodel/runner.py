    ...
    def _init_countries(self) -> dict[str, Country]:
        ...
        for country_name in self.config["model"]["country_names"]["value"]:
            ...
            economy = self._init_economy(
                ...
                initial_total_taxes_on_products=central_gov.ts.current("taxes_on_products")[0],
                initial_change_in_firm_stock_inventories=firms.ts.current("total_inventory_change").sum()
                ...
                initial_total_operating_surplus_plus_wages=firms.ts.current(
                    "gross_operating_surplus_mixed_income"
                ).sum()
                + firms.ts.current("total_wage").sum(),
                initial_individual_activity=individuals.states["Activity Status"],
                ...
                initial_gov_consumption=gov_entities.ts.current("total_consumption")[0],
                initial_cg_rent_received=central_gov.ts.current("total_rent_received")[0],
                initial_cg_taxes_rental_income=central_gov.ts.current("taxes_rental_income")[0],
