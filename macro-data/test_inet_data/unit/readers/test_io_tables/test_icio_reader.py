...
class TestICIOReader:
    ...
    def test__industries(self, readers):
        ...
        try:
            readers["icio"][2014].iot.loc[readers["icio"][2014].considered_countries[0]].loc[
                readers["icio"][2014].industries
            ]
        except KeyError:
    ...
    def test__import_export(self, readers):
        total_exports = reduce(
            lambda a, b: a + b,
            [
                readers["icio"][2014].get_exports(country)
                for country in readers["icio"][2014].considered_countries + ["ROW"]
            ],
        )
        total_imports = reduce(
            lambda a, b: a + b,
            [
                readers["icio"][2014].get_imports(country)
                for country in readers["icio"][2014].considered_countries + ["ROW"]
            ],
        )
