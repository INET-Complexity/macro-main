...
from data.util.download import download_data
from data.readers.handle_readers import get_reader_names, init_readers
from data.util.country_code_map import get_map_long_to_short
from data.readers.util.matching_iot_with_sea import (
    compile_industry_data,
    compile_exogenous_industry_data,
)
from data.util.regressions import fit_linear
from data.util.partition import partition_into_quintiles
...
from data.processing.synthetic_firms.synthetic_firms import SyntheticFirms
from data.processing.synthetic_firms.default_synthetic_firms import (
    SyntheticDefaultFirms,
)

from data.processing.synthetic_government_entities.synthetic_government_entities import (
...
from data.processing.synthetic_central_government.synthetic_central_government import (
...
from data.processing.synthetic_banks.synthetic_banks import SyntheticBanks
from data.processing.synthetic_banks.default_synthetic_banks import (
...
from data.processing.synthetic_central_bank.default_synthetic_central_bank import (
    SyntheticDefaultCentralBanks,
)

from data.processing.synthetic_credit_market.synthetic_credit_market import (
...
from data.processing.synthetic_matching.matching_individuals_with_firms import (
    match_individuals_with_firms,
)
from data.processing.synthetic_matching.matching_firms_with_banks import (
    match_firms_with_banks,
)
from data.processing.synthetic_matching.matching_households_with_banks import (
    match_households_with_banks,
)
from data.processing.synthetic_matching.matching_households_with_houses import (
...
class Creator:
    def __init__(
        ...
        self.country_names_short = list(
            map(get_map_long_to_short(self.raw_data_path).get, self.country_names)
        )
        ...
        for country_name in exogenous_industry_data.keys():
            if country_name in self.country_names:
                self.exogenous_data[country_name] = {
                    "log_inflation": self.data_readers["world_bank"].get_log_inflation(
                        country_name
                    ),
                    "sectoral_growth": self.data_readers[
                        "eurostat"
                    ].get_perc_sectoral_growth(country_name),
                    "unemployment_rate": self.data_readers[
                        "oecd_econ"
                    ].get_unemployment_rate(country_name),
                    "house_price_index": self.data_readers[
                        "oecd_econ"
                    ].get_house_price_index(country_name),
                    "vacancy_rate": self.data_readers["oecd_econ"].get_vacancy_rate(
                        country_name
                    ),
                    "total_firm_deposits_and_debt": self.data_readers[
                        "eurostat"
                    ].get_total_industry_debt_and_deposits(country_name),
                }
            else:
                self.exogenous_data[country_name] = {}
            self.exogenous_data[country_name][
    ...
    def create_synthetic_banks(self) -> None:
        for country_name in self.country_names:
            self.synthetic_banks[country_name] = SyntheticDefaultBanks(
                ...
                number_of_banks=(
                    ...
                    if self.config["model"]["single_bank"]["value"]
                        ...
                        1,
                        int(
                            self.data_readers["oecd_econ"].read_number_of_bank_branches(
            ...
            self.synthetic_banks[country_name].create(
                bank_equity=self.data_readers["eurostat"].get_total_bank_equity(
                    country_name, self.year
                )
            )
    ...
    def create_synthetic_central_gov(self, year_range: int = 10) -> None:
        for country_name in self.country_names:
            ...
            # Adding data
            unemp = [
                self.data_readers["oecd_econ"].unemployment_benefits_gdp_pct(
                    country_name, year
                )
                * self.data_readers["world_bank"].get_current_monthly_gdp(
                    country_name, year
                )
            ...
            self.synthetic_central_gov[country_name].create(
                central_gov_debt=self.data_readers["oecd_econ"].general_gov_debt(
                    country_name, self.year
                ),
    ...
    def create_synthetic_gov_entities(self) -> None:
        for country_name in self.country_names:
            ...
            if self.exogenous_data[country_name] is None:
                ...
                total_gov_consumption_growth = (
                    total_gov_consumption / total_gov_consumption.shift(1)
                ).values
            self.synthetic_gov_entities[country_name].create(
                single_government_entity=self.config["model"][
                    "single_government_entity"
                ]["value"],
                monthly_govt_consumption_in_usd=self.industry_data[country_name][
                    "industry_vectors"
                ]["Government Consumption in USD"].values,
                monthly_govt_consumption_in_lcu=self.industry_data[country_name][
                    "industry_vectors"
                ]["Government Consumption in LCU"].values,
                total_monthly_value_added_in_lcu=self.industry_data[country_name][
                    "industry_vectors"
                ]["Value Added in LCU"].sum(),
                total_number_of_firms=int(
                    self.data_readers["oecd_econ"]
                    .read_business_demography(
                        country=country_name,
                        output=pd.Series(
                            self.industry_data[country_name]["industry_vectors"][
                                "Output"
                            ].values
                        ),
                        year=self.year,
    ...
    def create_synthetic_central_bank(self) -> None:
        for country_name in self.country_names:
            ...
            self.synthetic_central_banks[country_name].create(
                initial_policy_rate=self.data_readers["policy_rates"].cb_policy_rate(
                    country_name, self.year
                )
            )
    ...
    def create_synthetic_population(self) -> None:
        for ind, country_name in enumerate(self.country_names):
            ...
            self.synthetic_population[country_name].create(
                ...
                n_households=int(
                    self.data_readers["eurostat"].number_of_households(
                        country_name, self.year
                    )
                    / self.scale
                ),
                number_of_firms_by_industry=self.industry_data[country_name][
                    "industry_vectors"
                ]["Number of Firms"].values,
                total_unemployment_benefits=self.synthetic_central_gov[country_name]
                .central_gov_data["Total Unemployment Benefits"]
                .values[0],
                rent_as_fraction_of_unemployment_rate=self.config["init"][country_name][
                    "central_government"
                ]["functions"]["social_housing"]["parameters"][
            ...
            self.synthetic_population[country_name].set_consumption_weights(
                consumption_weights=self.industry_data[country_name][
                    "industry_vectors"
                ]["Household Consumption Weights"].values
            )
    ...
    def create_synthetic_firms(self) -> None:
        for country_name in self.country_names:
            ...
            self.synthetic_firms[country_name].create(
                total_firm_deposits=self.data_readers[
                    "eurostat"
                ].get_total_nonfin_firm_deposits(country_name, self.year),
                total_firm_debt=self.data_readers[
                    "eurostat"
                ].get_total_nonfin_firm_debt(country_name, self.year),
                econ_reader=self.data_readers["oecd_econ"],
                ons_reader=self.data_readers["ons"],
                exchange_rates=self.data_readers["exchange_rates"],
                industry_data=self.industry_data[country_name],
                number_of_employees_by_industry=self.synthetic_population[
                    country_name
                ].number_employees_by_industry,
                initial_inventory_to_production_fraction=self.config["init"][
                    country_name
                ]["firms"]["functions"]["target_production"]["parameters"][
                    "target_inventory_to_production_fraction"
                ][
                    "value"
                ],
                intermediate_inputs_utilisation_rate=self.config["init"][country_name][
                    "firms"
                ]["parameters"]["intermediate_inputs_utilisation_rate"]["value"],
                capital_inputs_utilisation_rate=self.config["init"][country_name][
                    "firms"
                ]["parameters"]["capital_inputs_utilisation_rate"]["value"],
                assume_zero_initial_deposits=self.config["init"][country_name]["firms"][
                    "parameters"
                ]["assume_zero_initial_deposits"]["value"],
                assume_zero_initial_debt=self.config["init"][country_name]["firms"][
                    "parameters"
                ]["assume_zero_initial_debt"]["value"],
            )
    ...
    def create_synthetic_rest_of_the_world(self) -> None:
        ...
        if "ROW" in self.exogenous_data.keys():
            row_exports_data = (
                self.exogenous_data["ROW"]["iot_industry_data"]
                .xs("Exports in USD", axis=1, level=0)
                .sum(axis=1)
            )
            row_exports_data = row_exports_data.loc[
                row_exports_data.index < pd.Timestamp(self.year, 1, 1)
            ]
            row_exports_data_growth = (
                row_exports_data / row_exports_data.shift(1)
            ).values
            row_imports_data = (
                self.exogenous_data["ROW"]["iot_industry_data"]
                .xs("Imports in USD", axis=1, level=0)
                .sum(axis=1)
            )
            row_imports_data = row_imports_data.loc[
                row_imports_data.index < pd.Timestamp(self.year, 1, 1)
            ]
            row_imports_data_growth = (
                row_imports_data / row_imports_data.shift(1)
            ).values
        else:
        ...
        self.synthetic_rest_of_the_world.create(
            ...
            exchange_rate_usd_to_lcu=self.data_readers[
                "exchange_rates"
            ].from_usd_to_lcu("ROW", self.year),
    ...
    def set_household_saving_rates(self) -> None:
        for country_name in self.country_names:
            self.synthetic_population[country_name].set_household_saving_rates(
                independents=self.config["init"][country_name]["households"][
                    "functions"
                ]["saving_rates"]["parameters"]["independents"]["value"],
            )

    def normalise_household_consumption(
        self, positive_saving_rates_only: bool = True
    ) -> None:
        def default_desired_consumption(
            ...
            return (
                1.0
                / (1 + tau_vat_)
                * np.outer(consumption_weights_, (1 - saving_rates_) * income_).T
            )

        for country_name in self.country_names:
            iot_hh_consumption = self.industry_data[country_name]["industry_vectors"][
            ...
            income = (
                self.synthetic_population[country_name].household_data["Income"].values
            )
            sr = (
                self.synthetic_population[country_name]
                .household_data["Saving Rate"]
                .values
            )
            ...
            self.synthetic_population[country_name].household_data["Saving Rate"] = (
                ...
                - (
                    1
                    - self.synthetic_population[country_name].household_data[
                        "Saving Rate"
                    ]
                )
                * factor
    ...
    def match_consumption_weights_by_income(
        self, consumption_variance: float = 0.1
    ) -> None:
        def default_desired_consumption(
            income_: np.ndarray,
            consumption_weights_: np.ndarray,
        ...
        for country_name in self.country_names:
            ...
            quintiles = partition_into_quintiles(
                data=self.synthetic_population[country_name].household_data["Income"]
            )
            vat = self.data_readers["world_bank"].get_tau_vat(country_name, self.year)
            income = (
                self.synthetic_population[country_name].household_data["Income"].values
            ...
            def func(consumption_scalars):
                ...
                for _g in range(18):
                    i_cons_weights[0][_g] = iot_hh_consumption_norm[
                        _g
                    ] + consumption_scalars[_g] * (
                        weights_by_income[_g, 0] - consumption_scalars[_g + 18]
                    )
                    i_cons_weights[1][_g] = iot_hh_consumption_norm[
                        _g
                    ] + consumption_scalars[_g] * (
                        weights_by_income[_g, 1] - consumption_scalars[_g + 18]
                    )
                    i_cons_weights[2][_g] = iot_hh_consumption_norm[
                        _g
                    ] + consumption_scalars[_g] * (
                        weights_by_income[_g, 2] - consumption_scalars[_g + 18]
                    )
                    i_cons_weights[3][_g] = iot_hh_consumption_norm[
                        _g
                    ] + consumption_scalars[_g] * (
                        weights_by_income[_g, 3] - consumption_scalars[_g + 18]
                    )
                    i_cons_weights[4][_g] = iot_hh_consumption_norm[
                        _g
                    ] + consumption_scalars[_g] * (
                ...
                eqs.append(
                    np.sum(np.absolute(consumption_scalars[0:18]))
            ...
            self.synthetic_population[
                country_name
            ].consumption_weights_by_income = np.zeros((5, 18))
            for g_ in range(18):
                self.synthetic_population[country_name].consumption_weights_by_income[
                    0
                ][g_] = iot_hh_consumption_norm[g_] + res[g_] * (
    ...
    def match_firms_with_individuals(self) -> None:
        for country_name in self.country_names:
            ...
            match_individuals_with_firms(
                ...
                employee_social_contribution_taxes=self.data_readers[
                    "oecd_econ"
                ].read_tau_siw(country_name, self.year),
                income_taxes=self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                ),
            )
            ...
            self.synthetic_population[country_name].set_individual_labour_inputs(
                firm_production=self.synthetic_firms[country_name].firm_data[
                    "Production"
                ],
                firm_employees=self.synthetic_firms[country_name].firm_data[
                    "Employees ID"
                ],
            )
    ...
    def match_households_with_houses(self) -> None:
        for country_name in self.country_names:
            match_households_with_houses(
                ...
                rental_income_taxes=self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                ),
                social_housing_rent=self.synthetic_population[
                    country_name
                ].social_housing_rent,
                total_imputed_rent=self.data_readers["icio"][self.year].imputed_rents[
                    country_name
                ],
            )
    ...
    def compute_household_wealth_and_income(self) -> None:
        for country_name in self.country_names:
            self.synthetic_population[country_name].compute_household_wealth(
                wealth_distribution_independents=self.config["init"][country_name][
                    "households"
                ]["functions"]["wealth"]["parameters"]["independents"]["value"],
            )
            self.synthetic_population[country_name].compute_household_income(
                central_gov_config=self.config["init"][country_name][
                    "central_government"
                ],
    ...
    def compute_bank_initial_conditions(self) -> None:
        for country_name in self.country_names:
            self.synthetic_banks[country_name].set_initial_bank_fields(
                firm_deposits=self.synthetic_firms[country_name]
                .firm_data["Deposits"]
                .values,
                firm_debt=self.synthetic_firms[country_name].firm_data["Debt"].values,
                household_deposits=self.synthetic_population[country_name]
                .household_data["Wealth in Deposits"]
                .values,
                ...
                bank_markup_interest_rate_short_term_firm_loans=self.data_readers[
                    "eurostat"
                ].firm_risk_premium(country_name, self.year),
                bank_markup_interest_rate_long_term_firm_loans=self.data_readers[
                    "eurostat"
                ].firm_risk_premium(country_name, self.year),
                bank_markup_interest_rate_household_payday_loans=self.data_readers[
                    "eurostat"
                ].firm_risk_premium(country_name, self.year),
                bank_markup_interest_rate_household_consumption_loans=self.config[
                    "init"
                ][country_name]["banks"]["parameters"][
                    "initial_markup_interest_rate_household_consumption_loans"
                ][
                    "value"
                ],
                bank_markup_interest_rate_mortgages=self.config["init"][country_name][
                    "banks"
                ]["parameters"]["initial_markup_mortgage_interest_rate"]["value"],
                bank_markup_interest_rate_overdraft_firm=self.data_readers[
                    "eurostat"
                ].firm_risk_premium(country_name, self.year),
                bank_markup_interest_rate_overdraft_household=self.config["init"][
                    country_name
                ]["banks"]["parameters"][
    ...
    def set_initial_loans(self) -> None:
        for country_name in self.country_names:
            ...
            self.synthetic_credit_market[country_name].create(
                ...
                initial_firm_debt=self.synthetic_firms[country_name]
                .firm_data["Debt"]
                .values,
                ...
                .household_data["Outstanding Balance of Mortgages on other Properties"]
                .values,
                firms_corresponding_bank=self.synthetic_firms[country_name].firm_data[
                    "Corresponding Bank ID"
                ],
                households_corresponding_bank=self.synthetic_population[
                    country_name
                ].household_data["Corresponding Bank ID"],
                initial_firm_loan_maturity=self.config["init"][country_name]["banks"][
                    "parameters"
                ]["long_term_firm_loan_maturity"]["value"],
                household_consumption_loan_maturity=self.config["init"][country_name][
                    "banks"
                ]["parameters"]["household_consumption_expansion_loan_maturity"][
                    "value"
                ],
                mortgage_maturity=self.config["init"][country_name]["banks"][
                    "parameters"
                ]["mortgage_maturity"]["value"],
                assume_zero_initial_firm_debt=self.config["init"][country_name][
                    "firms"
                ]["parameters"]["assume_zero_initial_debt"]["value"],
            )
            self.synthetic_population[country_name].set_debt_installments(
                credit_market_data=self.synthetic_credit_market[
                    country_name
                ].credit_market_data
            )
    ...
    def update_central_gov_fields(self) -> None:
        for country_name in self.country_names:
            self.synthetic_central_gov[country_name].update_fields(
                total_social_housing_rent=self.synthetic_population[
                    country_name
                ].social_housing_rent
                * np.sum(
                    self.synthetic_population[country_name].household_data[
                ...
                firm_taxes_and_subsidies=float(
                    np.sum(
                        self.synthetic_firms[country_name].firm_data[
                ...
                firm_employer_si_tax=self.data_readers["oecd_econ"].read_tau_sif(
                    country_name, self.year
                )
                * np.sum(
                    self.synthetic_population[country_name].individual_data[
                        "Employee Income"
                    ]
                ),
                household_vat=self.data_readers["world_bank"].get_tau_vat(
                    country_name, self.year
                )
                * self.industry_data[country_name]["industry_vectors"][
                    "Household Consumption in LCU"
                ].sum(),
                export_tax=self.data_readers["world_bank"].get_tau_exp(
                    country_name, self.year
                )
                * self.industry_data[country_name]["industry_vectors"][
                    "Exports in LCU"
                ].sum(),
                employee_si_tax=self.data_readers["oecd_econ"].read_tau_siw(
                    country_name, self.year
                )
                * np.sum(
                    self.synthetic_population[country_name].individual_data[
                ...
                income_tax=self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                )
                * (
                    1
                    - self.data_readers["oecd_econ"].read_tau_siw(
                        country_name, self.year
                    )
                )
                * np.sum(
                ...
                + self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                )
                * np.sum(
                    self.synthetic_population[country_name].household_data["Rent Paid"]
                )
                + self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                )
                * np.sum(
                ...
                rental_income_tax=self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                )
                * np.sum(
                    self.synthetic_population[country_name].household_data["Rent Paid"]
                ),
                cf_tax=self.data_readers["eurostat"].taxrate_on_capital_formation(
                    country_name, self.year
                ),
            )
    ...
    def insert_industry_parameters(
        ...
        parameters[country_name]["industry_vectors"][
            "Number of Firms"
        ] = self.synthetic_firms[country_name].number_of_firms_by_industry
        parameters[country_name]["industry_vectors"][
            "Number of Employees"
        ] = self.synthetic_population[country_name].number_employees_by_industry
    ...
    def insert_taxes(
        ...
        parameters[country_name]["Taxes"] = pd.DataFrame(
            data={
                "Value-added Tax": self.data_readers["world_bank"].get_tau_vat(
                    country_name, self.year
                ),
                "Export Tax": self.data_readers["world_bank"].get_tau_exp(
                    country_name, self.year
                ),
                "Employer Social Insurance Tax": self.data_readers[
                    "oecd_econ"
                ].read_tau_sif(country_name, self.year),
                "Employee Social Insurance Tax": self.data_readers[
                    "oecd_econ"
                ].read_tau_siw(country_name, self.year),
                "Profit Tax": self.data_readers["oecd_econ"].read_tau_firm(
                    country_name, self.year
                ),
                "Income Tax": self.data_readers["oecd_econ"].read_tau_income(
                    country_name, self.year
                ),
                "Capital Formation Tax": self.data_readers[
                    "eurostat"
                ].taxrate_on_capital_formation(country_name, self.year),
            },
            index=[self.year],
        )
    ...
    def save_households(self, store: pd.HDFStore, country_name: str) -> None:
        ...
        corr_individuals = self.synthetic_population[country_name].household_data[
            "Corresponding Individuals ID"
        ]
        corr_renters = self.synthetic_population[country_name].household_data[
        ...
        household_data_without_lists = self.synthetic_population[
            country_name
        ].household_data
        ...
        store[
            country_name + "_synthetic_households"
        ] = household_data_without_lists.astype(float).rename_axis("Household ID")
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            store[
                country_name + "_synthetic_households_corr_individuals"
            ] = corr_individuals.rename_axis("Household ID")
            store[
                country_name + "_synthetic_households_corr_renters"
            ] = corr_renters.rename_axis("Household ID")
            store[
        ...
        store[
            ...
            data=self.synthetic_population[
                country_name
            ].consumption_weights_by_income.T,
            index=pd.Index(self.industries, name="Industry"),
    ...
    def save_firms(self, store: pd.HDFStore, country_name: str) -> None:
        ...
        store[country_name + "_synthetic_firms"] = firm_data_without_employees.astype(
            float
        ).rename_axis("Firm ID")
        ...
        store[
            ...
            index=pd.Index(
                range(len(self.synthetic_firms[country_name].firm_data)), name="Firm ID"
            ),
    ...
    def save_banks(self, store: pd.HDFStore, country_name: str) -> None:
        corr_firms = self.synthetic_banks[country_name].bank_data[
            "Corresponding Firms ID"
        ]
        corr_households = self.synthetic_banks[country_name].bank_data[
        ...
        store[country_name + "_synthetic_banks"] = bank_data_without_corr.astype(
            float
        ).rename_axis("Bank ID")
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
        ...
        store[country_name + "_policy_rate_markup"] = pd.DataFrame(
            data=[
                self.data_readers["eurostat"].firm_risk_premium(country_name, self.year)
            ],
        )
        ...
        store[country_name + "_long_term_interest_rates"] = pd.DataFrame(
            data=[
                (
                    1.0
                    + self.data_readers["oecd_econ"].read_long_term_interest_rates(
                        country_name, self.year
                    )
                )
                ** (1.0 / 12)
    ...
    def save_housing_market(self, store: pd.HDFStore, country_name: str) -> None:
        store[country_name + "_synthetic_housing_market"] = (
            self.synthetic_housing_market[country_name].housing_market_data.astype(
                float
            )
    ...
    def save_exogenous(self, store: pd.HDFStore, country_name: str) -> None:
        ...
        for field in [
            ...
            if field in self.exogenous_data[country_name].keys():
                store[country_name + "_exogenous_" + field] = self.exogenous_data[
                    country_name
                ][field]

    def save_rest_of_the_world(self, store: pd.HDFStore) -> None:
        store["synthetic_rest_of_the_world"] = (
            self.synthetic_rest_of_the_world.row_data.astype(float)
        ).rename_axis("Industry")
    ...
    def save(self) -> None:
        ...
        store["goods_criticality_matrix"] = self.data_readers[
            "goods_criticality"
        ].criticality_matrix
        ...
        for country_name in self.country_names:
            ...
            for param_key in parameters[country_name].keys():
                store[country_name + "_" + param_key] = parameters[country_name][
                    param_key
                ].astype(float)
