...
def check_existing_processed_data(config: dict, data_path: Path) -> Optional[str]:
    ...
    for filename in os.listdir(curr_path):
        ...
        config_init = pd.DataFrame(
            pd.read_hdf(curr_path / str(filename) / "data.h5", "config_init")
        ).values[0][0]
        ...
        for country_name in config_init.keys():
            ...
            for agent in config_init[country_name].keys():
                ...
                if "parameters" in config_init[country_name][agent].keys():
                    for parameter in config_init[country_name][agent][
                        "parameters"
                    ].keys():
                        if (
                            parameter
                            not in config["init"][country_name][agent][
                                "parameters"
                            ].keys()
                        ):
                            is_matching = False
                            break
                        if str(
                            config_init[country_name][agent]["parameters"][parameter][
                                "value"
                            ]
                        ) != str(
                            config["init"][country_name][agent]["parameters"][
                                parameter
                            ]["value"]
                        ):
                            is_matching = False
                ...
                if "functions" in config_init[country_name][agent].keys():
                    for function in config_init[country_name][agent][
                        "functions"
                    ].keys():
                        if (
                            function
                        ...
                        if (
                            "name"
                            in config_init[country_name][agent]["functions"][
                                function
                            ].keys()
                        ):
                            if str(
                                config_init[country_name][agent]["functions"][function][
                                    "name"
                                ]["value"]
                            ) != str(
                                config["init"][country_name][agent]["functions"][
                                    function
                                ]["name"]["value"]
                            ):
                                is_matching = False
                        ...
                        # Parameters
                        for parameter in config_init[country_name][agent]["functions"][
                            function
                        ]["parameters"].keys():
                            if (
                                parameter
                                not in config["init"][country_name][agent]["functions"][
                                    function
                                ]["parameters"].keys()
                            ):
                                is_matching = False
