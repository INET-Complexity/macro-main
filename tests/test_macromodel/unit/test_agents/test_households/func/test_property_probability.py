...
class TestHouseholdProbabilityBuying:
    ...
    def test_old_code_invalid_probabilities(self):
        ...
        diff_exp2 = np.exp(
            cost_comparison_temperature
            * (annual_cost_of_renting2 - annual_cost_of_purchasing2)
        )
    ...
    def test_comparison_old_vs_new(self):
        ...
        for description, rent_cost, buy_cost, expected in scenarios:
            ...
            with np.errstate(over="ignore", divide="ignore"):
                diff_exp_old = np.exp(
                    cost_comparison_temperature * (rent_cost - buy_cost)
                )
    ...
    def test_logistic_function_properties(self):
        ...
        assert np.isclose(
            prob_pos + prob_neg, 1.0, atol=0.01
        ), "Logistic function should be symmetric"
        ...
        for i in range(len(probs) - 1):
            assert (
                probs[i] <= probs[i + 1]
            ), "Probability should increase with cost difference"
